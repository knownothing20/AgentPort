...
当用户请求连接新服务器时，AI **必须先调用 `remote_ssh_info`**，然后根据扫描结果引导用户选择认证方式。

**标准流程：**

```
用户: 帮我连接 192.168.1.100

AI: 好的，我先扫描一下你本地的 SSH 环境，看看有哪些可用的资源。
    [调用 remote_ssh_info]

情况 A：有可用密钥和配置
    扫描结果：
    ✅ 发现 2 个私钥：id_ed25519（未加密）、id_rsa（已加密）
    ✅ SSH config 中有匹配主机 'dev' → 192.168.1.100:22
    ✅ known_hosts 中已记录此主机
...
情况 B：无可用密钥
    ...
    2. 生成新的 SSH 密钥对（需要后续将公钥添加到服务器）

情况 C：SSH config 已有完整配置
    扫描结果：
    ✅ SSH config 中发现 'prod' → 192.168.1.100:22 user=root key=~/.ssh/id_ed25519
...
- 没有密钥时提供两个选项：密码登录 或 生成新密钥

#### 2. 分步提问，一次只问一个参数

AI 在引导用户配置连接时，**必须一次只询问一个参数**，不要一次性列出所有问题。
推荐提问顺序：host → remote_ssh_info（自动扫描）→ 认证方式选择 → password/privateKey → workspaceRoot（可选）
...
AI: 用什么方式登录？（推荐 SSH 密钥）
用户: 用密码

AI: 好的，请输入登录密码
用户: ********

AI: 远程工作区想放在哪里？（默认 /home/yourname/workspace）
    💡 工作区 = 安全边界，所有文件操作只能在这个目录内进行
用户: 就默认

AI: [远程执行命令，自动生成 Token]
    ✅ SSH 连接测试成功
    ⏳ 正在部署守护进程...
    ✅ 守护进程已启动

    **连接信息：**
    - SSH: yourname@192.168.1.100:22
    - Daemon: http://192.168.1.100:3183
    - 工作区: /home/yourname/workspace

    📊 Dashboard: http://192.168.1.100:3183/?token=xxx
